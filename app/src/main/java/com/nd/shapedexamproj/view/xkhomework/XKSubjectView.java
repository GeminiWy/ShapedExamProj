 ...
 */
public abstract class XKSubjectView extends LinearLayout {

    protected XKSubject subject;
    protected AutoImageLoadTextView titleView;
    protected WeakReference<XKINextable> nextWeakReference;

    public XKSubjectView(Context context) {
        super(context);
    }

    public XKSubjectView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void showCompexSubjectType() {
        StringBuffer stringBuffer = new StringBuffer(subject.getTheSubjectTypeIndex()
                + ").【" + XKSubject.getTypeName(subject.getType()) + "】"
                + subject.getSubjectName());
        if (subject.getScore() >= 0) {
            stringBuffer.append(" <font color=\"#888888\">(" + Utils.scoreFloat2String(subject.getScore()) + "分)</font>");
        }
     ...
     * @param nextable
     */
    public void setShowNext(XKINextable nextable) {
        nextWeakReference = new WeakReference<XKINextable>(nextable);
    }
