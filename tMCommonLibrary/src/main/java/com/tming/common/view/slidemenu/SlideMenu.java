...
public class SlideMenu extends ViewGroup {
	...
	private float mPressedX, mPressedY;
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mLastMotionX = mPressedX = event.getX();
                mPressedY = event.getY();
                mIsTapContent = isTapContent(mPressedX, mPressedY);
                return mIsTapContent && isOpen();
            case MotionEvent.ACTION_MOVE:
                moveX = event.getX() - mPressedX;
                moveY = event.getY() - mPressedY;
                if (Math.abs(moveX) > mTouchSlop) {
                    if (mIsTapContent && Math.abs(moveX) > Math.abs(moveY)
                    		&& !canScroll(this, (int) moveX, (int) event.getX(), (int) event.getY())) {
