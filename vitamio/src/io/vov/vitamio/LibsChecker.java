...
public final class LibsChecker {
	...
	public static final boolean checkVitamioLibs(final Activity ctx) {
		if (!Vitamio.isInitialized(ctx)){
//				&& !ctx.getIntent().getBooleanExtra(FROM_ME, false)) {
//			Intent i = new Intent();
//			i.setClassName(Vitamio.getVitamioPackage(),
...
//			return false;
			new Thread(){
				public void run() {
					Vitamio.initialize(ctx, R.raw.libarm);
				};
			}.start();
		}
		return true;
